...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static Maze maze; // the user must load in this maze
    private static double cellWidth;
    private static double cellHeight;
    private static ArrayList<Cell> selectedCells = new ArrayList();
    // the following two variables will be used in erasing the last path traversed
    private static ArrayList<Integer> lastSetOfDirectionsUsed;
    private static Cell lastStartCellUsed;
    ...
    *************************************************************************/
    ...
    public void mouseClicked(MouseEvent e) { 
        StdDraw.highlightCell(StdDraw.userX(e.getX()), StdDraw.userY(e.getY()));
    }
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            ...
            else
            {
                ...
                setPenColor(new Color(rand.nextFloat(), rand.nextFloat(),rand.nextFloat(),rand.nextFloat()));
            }
            */
    ...
    *************************************************************************/
    ...
    /** Get Frame | Marko Sterbentz 4/18/2015
    ...
    /********************************************************************
     * MAZE RELATED METHODS BELOW
     ********************************************************************/
    
    /** Draw Maze | Marko Sterbentz 4/18/2015
     *  This method will draw the given Maze object.
     */
    public static void drawMaze()
    {
        Cell[][] cells = maze.getCells();
        double x = 0.05; // x and y together represent the top corner of each cell
        double y = 0.95;
        ...
        for(int i = 0; i < cells.length; i++)
        {
            ...
            {
                drawCell(cells[i][j], cellWidth, cellHeight, x, y);
                x += cellWidth;
            }
            y -= cellHeight;
            x = 0.05;
    ...
    /** Draw Cell | Marko Sterbentz 4/18/2015
     *  This method will draw an individual cell in the given position.
     */
    private static void drawCell(Cell cell, double wallWidth, double wallHeight, double x, double y)
    {
        if(cell.walls[0])
            StdDraw.line(x, y, (x + wallWidth), y);
        if(cell.walls[1])
            StdDraw.line((x + wallWidth), y, (x + wallWidth), (y - wallHeight));     
        if(cell.walls[2])
            StdDraw.line((x + wallWidth), (y - wallHeight), x, (y - wallHeight));
        if(cell.walls[3])
            StdDraw.line(x, (y - wallHeight), x , y);
    }
    
    /** Draw Labels | Marko Sterbentz 4/20/2015
     *  This method will draw the START and END labels onto the maze.
     */
    public static void drawLabels()
    {
        StdDraw.setPenColor(RED);
        StdDraw.textLeft(0.01, 0.97, "START");
        StdDraw.textLeft(0.93, 0.02, "END");
        StdDraw.setPenColor();
    }
    
    /** Draw Path | Marko Sterbentz 4/20/2015
     *  This method will draw the path from the given startCell along the 
     *  path specified by the given directions.
     */
    public static void drawPath(Cell startCell, ArrayList<Integer> directions, Color penColor)
    {
        lastSetOfDirectionsUsed = directions; // will be used in order to erase this path later
        lastStartCellUsed = startCell; // will be used in order to erase this path later
        
        Cell[][] cells = maze.getCells();  
        Point2D.Double coordinates = getCoordinatesOfCell(startCell);
        double x = coordinates.x; // x and y together represent the top left corner of each cell
        double y = coordinates.y;
        
        double xCenter = x + (cellWidth / 2);
        double yCenter = y - (cellHeight / 2);       

        StdDraw.setPenColor(penColor);
        if(cellWidth <= cellHeight)
            StdDraw.setPenRadius(cellWidth / 10);
        if(cellHeight < cellWidth)
            StdDraw.setPenRadius(cellHeight / 10);
        //if(penColor == StdDraw.WHITE)
           // StdDraw.setPenRadius(StdDraw.getPenRadius() + Math.min(cellHeight, cellWidth) / 9);
        for(int i = 0; i < directions.size(); i++)
        {
            if(directions.get(i) == 0)
            {
                StdDraw.line(xCenter, yCenter, xCenter, (yCenter + cellHeight));
                yCenter += cellHeight;
            }
            else if(directions.get(i) == 1)
            {
                StdDraw.line(xCenter, yCenter, (xCenter + cellWidth), yCenter);
                xCenter += cellWidth;
            }
    ...
    /** Highlight Cell | Marko Sterbentz 4/20/2015
     *  This method will highlight the cell at the given x and y coordinates.
     */
    private static ArrayList<Cell> highlightCell(double xClick, double yClick)
    {
        Cell[][] cells = maze.getCells();
        double x = 0.05; // x and y together represent the top left corner of each cell
        double y = 0.95;       
        int xIndex = 0;
        int yIndex = 0;
        
        while(x < xClick - cellWidth)
        {
            x += cellWidth; 
            xIndex++;
        }
        while(y > yClick + cellHeight)
        {
            y -= cellHeight;
            yIndex++;
        }
                
        if(selectedCells.contains(cells[yIndex][xIndex]))
        {
            StdDraw.eraseHighlight(x, y);
            selectedCells.remove(cells[yIndex][xIndex]);
        }
        else
        {
            StdDraw.setPenColor(BLUE);
            StdDraw.filledRectangle(x + (cellWidth / 2), y - (cellHeight / 2), cellWidth / 3, cellHeight / 3);
            // add cell[yIndex][xIndex] to selected cells list so that it can be manipulated 
            if(selectedCells.size() < 2)
                selectedCells.add(cells[yIndex][xIndex]);
            else 
            {
                StdDraw.eraseHighlight(selectedCells.get(0));
    ...
    /** Highlight Cell | Marko Sterbentz 4/20/2015
     *  This method will highlight the given cell.
     */
    private static void highlightCell(Cell cell)
    {
        Point2D.Double coordinates = getCoordinatesOfCell(cell);
    ...
    /** Erase Highlight | Marko Sterbentz 4/20/2015
     *  This method will fill the Cell with the given upper left corner
     *  with white.
     */
    private static void eraseHighlight(double x, double y)
    {
        StdDraw.setPenColor(WHITE);
        StdDraw.filledRectangle(x + (cellWidth / 2), y - (cellHeight / 2), cellWidth / 2.5, cellHeight / 2.5);
    }
    
    /** Erase Highlight | Marko Sterbentz 4/20/2015
     *  This method will fill the given Cell of the maze with white.
     */
    private static void eraseHighlight(Cell cell)
    {
        Point2D.Double cellCoordinates = getCoordinatesOfCell(cell);
        StdDraw.eraseHighlight(cellCoordinates.x, cellCoordinates.y);
    }
    
    /** Get Coordinates of Cell | Marko Sterbentz 4/20/2015
     *  This method will return the coordinates of the given cell.
     */
    private static Point2D.Double getCoordinatesOfCell(Cell cell)
    {
        ...
        for(int i = 0; i < maze.getCells().length; i++)
        {
            ...
            {
                if(cell == getCellAtCoordinates(x, y))
                {
                    coordinates.x = x;
                    coordinates.y = y;
                    return coordinates;
    ...
    /** Get Cell At Coordinates | Marko Sterbentz 4/20/2015
     *  This method will return the cell that is located at the given coordinates
     *  in the maze.
     */
    private static Cell getCellAtCoordinates(double xCoordinate, double yCoordinate)
    {
        double x = 0.05; // x and y together represent the top left corner of each cell
        double y = 0.95;       
        int xIndex = 0;
        ...
        return maze.getCells()[yIndex][xIndex];
    }
    
    /** Clear Paths | Marko Sterbentz 4/20/2015
     *  This method will erase all paths that currently exist in the maze.
     */
    public static void clearPath()
    {
        if(lastStartCellUsed != null && lastSetOfDirectionsUsed != null)
            StdDraw.drawPath(lastStartCellUsed, lastSetOfDirectionsUsed, WHITE);
        for(int i = 0; i < selectedCells.size(); i++)
                highlightCell(selectedCells.get(i));
    }
    
    /** Get Selected Cells | Marko Sterbentz 4/20/2015
     *  This method will return the Cells selected by the user.
     */
    ...
    /** Load in Maze | Marko Sterbentz 4/20/2015
     *  This method will load a given maze into StdDraw so that it can
     *  be drawn effectively.
     */
    public static void loadInMaze(Maze givenMaze)
    {
        maze = givenMaze;
        cellWidth = Math.min(0.9 / maze.getCells()[0].length, 0.9 / maze.getCells().length);
        cellHeight = cellWidth;//0.9 / maze.getCells().length;
    }
}
